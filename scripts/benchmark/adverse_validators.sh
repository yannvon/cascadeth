...
check_balances_peerview () {
  echo $2
  $geth_path attach $1 --exec $tx_str_1$node1_acc1$tx_str_2
}
...
echo "Start bootnode"
$bootnode_path --nodekey scripts/benchmark/bootnode.key &
bootnode=$!
...
for i in $(seq 0 5)
do
  $geth_path attach $ipc_str_1$i$ipc_str_2$i$ipc_str_3 --exec "admin.peers"
done
